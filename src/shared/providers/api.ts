...
instance.interceptors.response.use(
    function (response) {
        ...
        if (__DEV__) {
            console.log(`<<< [API Response] ${response.config.method?.toUpperCase()} ${response.config.baseURL + (response.config.url || '')}`,JSON.stringify(response.config.data),  JSON.stringify(response.data));
        }
